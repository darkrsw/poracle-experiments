...
def extract_result(filename, indexes, correctnesses, subjects):
...
def start_process(path, csv_file, model_info, sw=0):
    i = 0
    indexes = []
    correctnesses = []
    subjects = []
    for subdir, dirs, files in os.walk(path):
        for filename in files:
            if filename.endswith(".json"):
                if "PFL" in filename or "bug" in filename:
                    continue
    ...
    print(i)
    return extract_result(csv_file, indexes, correctnesses, subjects)
...
if __name__ == "__main__":
    ...
    file = 'cv_prediction_result.csv'
    c_all, in_all, in_all_correct = start_process(path, file, model_info,2)
